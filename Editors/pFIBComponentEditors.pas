...
begin
  ...
  try
    ...
    begin
      ...
      else if pName = 'CacheFileName' then
...
begin
  if ObjSupports(Component, IFIBConnect, DB) then
    case Index of
      0:
        if EditFIBDatabase(Component) then
          Designer.Modified;
      1:
        begin
          // try to connect
          ...
          if DB.QueryValueAsStr(qryExistTable, 0, ['FIB$FIELDS_INFO']) = '0'
          then
          begin
            if MessageDlg(SCompEditInfoFieldsNotExist, mtConfirmation,
      ...
      2:
        begin
          ...
          if DB.QueryValueAsStr(qryExistTable, 0, ['FIB$ERROR_MESSAGES']) = '0'
          then
          begin
            if MessageDlg(SCompEditErrorTableNotExist, mtConfirmation,
              [mbOK, mbCancel], 0) <> mrOk then
              Exit;
      ...
      3:
        begin
          ...
          if DB.QueryValueAsStr(qryExistTable, 0, ['FIB$DATASETS_INFO']) = '0'
          then
          begin
            if MessageDlg(SCompEditInfoTableNotExist, mtConfirmation,
              [mbOK, mbCancel], 0) <> mrOk then
              Exit;
...
begin
  case Index of
    0:
      Result := SCompEditDBEditor;
    1:
      Result := SCompEditEditFieldInfo;
    2:
      Result := SCompEditErrorMessages;
    3:
      Result := SCompEditDataSetInfo;
    4:
      Result := SRepositoriesScript;
  end;
end;
...
begin
  Conditions := TConditions(GetObjectProp(GetComponent(0), 'Conditions'));

  if Assigned(Conditions) then
    if EditConditions(Conditions) then
      Designer.Modified
end;
...
begin
  case Index of
    0:
      if EditFIBTrParams(Component) then
        Designer.Modified
  end;
...
begin
  case Index of
    0:
      Result := SCompEditEditTransaction;
  end;
end;
...
begin
  ...
  else if EditOptions(DataSets, 1) then
    Modified;
end;
  ...
  try
    try
      ...
      try
        ...
        begin
          Proc(Trim(VarToStr(iQry.FieldValue('RDB$PROCEDURE_NAME', False))));
          iQry.iNext;
        end;
        iQry.Close;
      finally
...
begin
  if PropCount > 1 then
    Exit;
  if not(THackAutoUpdateOptions(GetComponent(0)).FOwner is expDatasetClass) then
    Exit;
  ...
  try
    ...
    try
      ...
      while not iQry.iEof do
      begin
        Proc(Trim(VarToStr(iQry.FieldValue(0, False))));
        iQry.iNext;
      end;
      iQry.Close;
    finally
...
begin
  ...
  for i := VarArrayLowBound(Keys, 1) to VarArrayHighBound(Keys, 1) do
  begin
    v := DefReadFromRegistry(['Software', RegFIBRoot, RegFIBTrKinds, Keys[i]],
      ['Name']);
...
begin
  ...
  if Assigned(P) and (THackAutoUpdateOptions(GetComponent(0)).FOwner is TDataSet)
  then
    if ShowEdParamToFields(TDataSet(THackAutoUpdateOptions(GetComponent(0))
      .FOwner), TStrings(P)) then
      Designer.Modified
end;
...
begin
  ...
  if not ObjSupports(ds, IFIBDataSet, ids) then
    Exit;
  Obj := GetObjectProp(THackAutoUpdateOptions(GetComponent(0)).FOwner,
    'Database');
    ...
    try
      try
        ...
        if FieldCount > 0 then
          ...
          begin
            if (Fields[i] is TLargeIntField) or (Fields[i] is TIntegerField) or
              ((Fields[i] is TBCDField) and (TBCDField(Fields[i]).Size = 0)) and
              Stringer.EquelStrings(ids.GetRelationTableName(Fields[i]),
              updTableName, False) then
            begin
        ...
        begin
          ...
          begin
            if (FieldDefs[i].DataType in [ftSmallint, ftInteger]) and
              Stringer.EquelStrings(ids.GetRelationTableName(FieldDefs[i]),
              updTableName, False) then
            begin
              Proc(FieldDefs[i].Name);
            end
            else if ((FieldDefs[i].DataType = ftBCD) and (FieldDefs[i].Size = 0)
              ) and Stringer.EquelStrings(ids.GetRelationTableName(FieldDefs[i]
              ), updTableName, False) then
            begin
              Proc(FieldDefs[i].Name);
            end;
...
begin
  if PropCount > 1 then
    Exit;
  if not(THackAutoUpdateOptions(GetComponent(0)).FOwner is TDataSet) then
    Exit;
  Tables := TStringList.Create;
  try
    Supports(FIBClassesExporter, IFIBClassesExporter, clExp);
    ...
    Stringer.AllTables
      (TStrings(GetObjectProp(THackAutoUpdateOptions(GetComponent(0)).FOwner,
      'SelectSQL')).Text, Tables);
    for i := 0 to Pred(Tables.Count) do
      Proc(Stringer.ExtractWord(1, Tables[i], [' ']));
  finally
    Tables.free
  end
end;
...
procedure ShowDataSetSQLEditor(Component: TComponent; Kind: Integer;
  Designer: IDesigner);
const
    ...
    begin
      if ExitToCodeEditor <> k then
        if CloseModule(Component, SQLNames[k]) then
          CreatePropInCode(Component, SQLNames[k], nil, True);
      SaveModule(Component, SQLNames[k]);
    end;
    if ExitToCodeEditor >= 0 then
...
constructor TFIBGenSQlEd.Create(AComponent: TComponent;
...
begin
  if Index < DefaultEditor.GetVerbCount then
    ...
    case Index - DefaultEditor.GetVerbCount of
      0:
        Result := SCompEditSQLGenerator;
      1:
        Result := SCompSaveToDataSetInfo;
      2:
        Result := SCompChooseDataSetInfo;
      3:
        Result := SCheckSQLs;
    end;
end;
...
begin
  if ObjSupports(Component, IFIBDataSet, ids) then
    ...
    begin
      ...
      case Index - DefaultEditor.GetVerbCount of
        0:
          ShowDataSetSQLEditor(Component, 0, Designer);
        1:
          SaveDataSetInfo;
        2:
          begin
            ...
            *)
            ...
            if not Assigned(curObj) or not GetPropValue(curObj, 'Connected')
            then
            begin
              MessageDlg(SCompDatabaseNotConnected, mtConfirmation, [mbOK], 0);
              Exit;
            ...
            if idb.QueryValueAsStr(qryExistTable, 0, ['FIB$DATASETS_INFO']) = '0'
            then
            begin
              if MessageDlg(SCompEditInfoTableNotExist, mtConfirmation,
                [mbOK, mbCancel], 0) <> mrOk then
...
begin
  ...
  with Conditions do
    for i := 0 to Pred(Count) do
      Result := Result + Names[i] + ExchangeDelimeter + Values[Names[i]] +
        ExchangeDelimeter + IntToStr(Ord(THackCondition(Objects[i]).FEnabled)
        ) + #13#10
...
begin
  ...
  begin
    if idb.QueryValueAsStr
      ('SELECT Count(*) FROM FIB$DATASETS_INFO WHERE DS_ID=:DS_ID', 0, [DS_ID])
      = '0' then
      idb.Execute
        ('INSERT INTO FIB$DATASETS_INFO (DS_ID,UPDATE_ONLY_MODIFIED_FIELDS) VALUES('
        + IntToStr(DS_ID) + ',1)');
    idb.Execute('Update FIB$DATASETS_INFO SET '#13#10 + 'SELECT_SQL=''' +
      TStrings(GetObjectProp(DataSet, 'SelectSQL')).Text + ''','#13#10 +
      'INSERT_SQL=''' + TStrings(GetObjectProp(DataSet, 'InsertSQL')).Text +
      ''','#13#10 + 'UPDATE_SQL=''' + TStrings(GetObjectProp(DataSet,
      'UpdateSQL')).Text + ''','#13#10 + 'DELETE_SQL=''' +
      TStrings(GetObjectProp(DataSet, 'DeleteSQL')).Text + ''','#13#10 +
      'REFRESH_SQL=''' + TStrings(GetObjectProp(DataSet, 'RefreshSQL')).Text +
      ''','#13#10 + 'KEY_FIELD=''' + GetPropStringValue(DataSet,
      'AutoUpdateOptions.KeyFields') + ''','#13#10 + 'NAME_GENERATOR=''' +
      GetPropStringValue(DataSet, 'AutoUpdateOptions.GeneratorName') +
      ''','#13#10 + 'DESCRIPTION=''' + GetPropStringValue(DataSet,
      'Description') + ''','#13#10 + 'UPDATE_TABLE_NAME=''' +
      GetPropStringValue(DataSet, 'AutoUpdateOptions.UpdateTableName') +
      ''','#13#10 + 'UPDATE_ONLY_MODIFIED_FIELDS=''' +
      IntToStr(GetSubPropValue(DataSet,
      'AutoUpdateOptions.UpdateOnlyModifiedFields')) + ''','#13#10 +
      'CONDITIONS=''' + GetConditionsExchangeStr(TStrings(GetObjectProp(DataSet,
      'Conditions'))) + ''''#13#10 +
...
begin
  ...
  if DB = nil then
    Exit
  else if ObjSupports(DB, IFIBConnect, idb) then
  begin
...
begin
  ...
  if pName = 'SelectSQL' then
    KindSQL := 0
  else if pName = 'UpdateSQL' then
    KindSQL := 2
  else if pName = 'InsertSQL' then
    KindSQL := 1
  else if pName = 'DeleteSQL' then
    KindSQL := 3
  else
    KindSQL := 4;
...
const
  QRYTables = 'SELECT REL.RDB$RELATION_NAME FROM RDB$RELATIONS REL WHERE ' +
    '(REL.RDB$SYSTEM_FLAG <> 1 OR REL.RDB$SYSTEM_FLAG IS NULL) AND ' +
    '(NOT REL.RDB$FLAGS IS NULL) AND ' + '(REL.RDB$VIEW_BLR IS NULL) AND ' +
  ...
  try
    try
      try
        ...
        while not iQry.iEof do
        begin
          Proc(Trim(VarToStr(iQry.FieldValue('RDB$RELATION_NAME', False))));
          iQry.iNext;
        end;
        iQry.Close;
      finally
    ...
    case Index of
      0:
        begin
          ...
          Script.Add('');
          Script.Add
            ('CREATE  DESCENDING INDEX IDX_FIB$COMMIT_NO ON FIB$TRANSACTIONS (COMMIT_NO);');
          Script.Add('');
          ...
          Script.Add
            ('CREATE PROCEDURE FIB$GET_TRANSACTION_ID ( FORCE   SMALLINT )');
      ...
      1:
        begin
          ...
          TriggerName := Stringer.FormatIdentifier(3,
            'FIB$TRANSLOG_' + TableName);
          TableName := Stringer.FormatIdentifier(3, TableName);
          Script.Add('ALTER TABLE ' + TableName +
            ' ADD FIB$TRANSACTION_ID FIB$BIGINT;');
          Script.Add('ALTER TABLE ' + TableName +
            ' ADD FIB$UPDATE_TIME FIB$TIMESTAMP;');
          Script.Add('');
          ...
          Script.Add('BEGIN');
          Script.Add
            ('  Select TRANS_ID From FIB$GET_TRANSACTION_ID(1) INTO :TRANS_ID;');
          Script.Add('  if (DELETING) then');
          Script.Add('   Insert into ' + DelTableName +
            '(ID,FIB$TRANSACTION_ID,FIB$DELETED_TIME)');
...
begin
  case Index of
    0:
      Result := SCompEditDeltaReceiver1;
    1:
      begin
        TableName := GetPropStringValue(Component, 'TableName');
