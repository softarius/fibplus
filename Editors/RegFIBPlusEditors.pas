...
type
  ...
  IFIBSQLTextEditor = interface
    ...
    procedure ScreenPosToTextPos(const ScrX, ScrY: Integer;
      var DestX, DestY: Integer); // для драг дропа
    ...
    property DoBeforeProposalCall: TiBeforeProposalCall read GetBeforePropCall
      write SetBeforePropCall;
...
uses pFIBComponentEditors, {$IFNDEF internal} fib_aspr_api, Fibmodule5,
  Fibmodule6, FibHash2, Fibhash3,
  fib_hard,
{$ENDIF}
...
type
  ...
  TIDEIOTAFibPlus = class(TNotifierObject, IOTANotifier, IOTAIDENotifier,
    IOTAIDENotifier50)
  public
    // IOTAIDENotifier
...
begin
  ...
  RegisterPropertyEditor(TypeInfo(string), DatabaseClass, 'AliasName',
    TFIBAliasEdit);
  RegisterPropertyEditor(TypeInfo(string), DatabaseClass, 'DBName',
    TFileNameProperty);
  RegisterPropertyEditor(TypeInfo(string), DatabaseClass, 'LibraryName',
    TFileNameProperty);
  RegisterPropertyEditor(TypeInfo(TNotifyEvent), DatabaseClass,
    'OnConnect', nil);
  ...
  RegisterComponentEditor(TComponentClass(TransactionClass),
  ...
  RegisterPropertyEditor(pi.PropType^, nil, 'Options',
    TpFIBDataSetOptionsEditor);
  pi := GetPropInfo(DatasetClass, 'PrepareOptions');
  RegisterPropertyEditor(pi.PropType^, nil, 'PrepareOptions',
    TpFIBDataSetOptionsEditor);
  pi := GetPropInfo(DatasetClass, 'AutoUpdateOptions');
  RegisterPropertyEditor(pi.PropType^, DatasetClass, 'AutoUpdateOptions',
    TpFIBAutoUpdateOptionsEditor);

  auClass := GetTypeData(pi^.PropType^).ClassType;
  RegisterPropertyEditor(TypeInfo(TStrings), auClass, 'ParamsToFieldsLinks',
    TEdParamToFields);
  RegisterPropertyEditor(TypeInfo(string), auClass, 'GeneratorName',
    TGeneratorNameEdit);

  RegisterPropertyEditor(TypeInfo(string), auClass, 'KeyFields',
    TKeyFieldNameEdit);

  RegisterPropertyEditor(TypeInfo(string), auClass, 'UpdateTableName',
    TTableNameEdit);

  RegisterPropertyEditor(TypeInfo(Integer), DatasetClass, 'DataSet_ID',
    TDataSet_ID_Edit);
  ...
  RegisterPropertyEditor(TypeInfo(boolean), DatasetClass,
    'WaitEndMasterScroll', nil);

  pi := GetPropInfo(DatasetClass, 'SQLs');
  RegisterPropertyEditor(TypeInfo(TStrings), GetTypeData(pi^.PropType^)
    .ClassType, '', TFIBSQLsProperty);
  ...
  auClass := GetTypeData(pi^.PropType^).ClassType;
  RegisterPropertyEditor(TypeInfo(TOwnedCollection), auClass, 'GeneratorList',
    TpFIBGeneratorsProperty);

  RegisterPropertyEditor(TypeInfo(string), auClass, 'CacheFileName',
    TFileNameProperty);
  ...
  if Assigned(expServicesClass) then
    RegisterPropertyEditor(TypeInfo(string), expServicesClass, 'LibraryName',
      TFileNameProperty);

  RegisterPropertyEditor(TypeInfo(string), DeltaReceiver, 'TableName',
    TTableNameEditDR);

  RegisterComponentEditor(TComponentClass(DeltaReceiver),
...
procedure TFIBSQLMemo.ScreenPosToTextPos(const ScrX, ScrY: Integer;
  var DestX, DestY: Integer); // для драг дропа
begin
  DestX := -1
end;
...
{$I include\userpolybuffer.inc}
  ...
  if Assigned(DemoTimer) then
    Msg := Msg + 'Trial version'
  else
    Msg := Msg + 'Registered on ' + LicensedTo;
  if Assigned(CheckLicense) then
    Msg := Msg + #13#10'Running instance: ' +
      IntToStr(CheckLicense.InstanceCount);
...
{$I include\userpolybuffer.inc}
  ...
  if Cancel then
    (BorlandIDEServices as IOTAMessageServices)
      .AddTitleMessage
      ('The compilation of the project has been stopped because of the violation of the license agreement: the current registration key is being used on multiple computers simultaneously.');
end;
...
procedure TIDEIOTAFibPlus.FileNotification(NotifyCode: TOTAFileNotification;
  const FileName: string; var Cancel: boolean);
begin
{$I include\userpolybuffer.inc}
  Cancel := not(Assigned(CheckLicense) and CheckLicense.IsValid);
...
{$I include\userpolybuffer.inc}
  if Assigned(DemoTimer) then
    FreeAndNil(DemoTimer);
  if Assigned(CheckLicense) then
    FreeAndNil(CheckLicense);
